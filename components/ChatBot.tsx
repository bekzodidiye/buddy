...
import { sendMessageToBuddy } from '../services/geminiService';
...
const ChatBot: React.FC = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const responseText = await sendMessageToBuddy(userMessage);
      setMessages(prev => [...prev, { role: 'model', text: responseText || "Kechirasiz, javob olishda xatolik yuz berdi." }]);
    } catch (error) {
    ...
    <div className="fixed bottom-6 right-6 z-[100]">
      ...
      {!isOpen && (
        <button
          ...
          className="w-16 h-16 bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-2xl buddy-glow flex items-center justify-center transition-all hover:scale-110 active:scale-95 group"
        >
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="w-[350px] sm:w-[400px] h-[550px] bg-[#121214] border border-white/10 rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
          {/* Header */}
